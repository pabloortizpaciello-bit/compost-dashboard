...
with st.sidebar:
    ...
    if uploaded:
        ...
        if range_mode == "Last N hours":
            ...
            coarse_start = t_max - timedelta(days=n_d)
        else:
            coarse_start = t_min
...
for col, accent in key_metrics:
    if col in df.columns:
        ...
        metrics_html += (
            ...
            f'<div class="metric-value">{val:.1f}'
            f'<span class="metric-unit">{info["unit"]}</span></div>'
            f'</div>'
        )
...
with tab2:
    st.markdown(
        ...
        f'{len(df):,} rows · '
        f'{t_start.strftime("%Y-%m-%d %H:%M")} → {t_end.strftime("%Y-%m-%d %H:%M")}'
        f'</span>',
        unsafe_allow_html=True,
    )
...
with tab3:
    ...
    fname = (
        f"compost_"
        f"{t_start.strftime('%Y%m%d_%H%M')}_"
        f"{t_end.strftime('%Y%m%d_%H%M')}.csv"
    )
    st.download_button(
        "⬇ Download CSV", data=csv_bytes, file_name=fname, mime="text/csv",
    )
    st.markdown(
        f'<div class="info-box">'
        f'<b>File:</b> {fname}<br>'
        f'<b>Rows:</b> {len(df):,}<br>'
        f'<b>Range:</b> {t_start.strftime("%Y-%m-%d %H:%M")} → '
